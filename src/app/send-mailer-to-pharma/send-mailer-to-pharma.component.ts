...
@Component({
  selector: 'app-send-mailer-to-pharma',
  templateUrl: './send-mailer-to-pharma.component.html',
  styleUrls: ['./send-mailer-to-pharma.component.css']
})
export class SendMailerToPharmaComponent implements OnInit {
  ...
  ngOnInit() {
   // alert("In end pharma page")
    ...
    this.getData(this.id_of_event);
  }

  getData(id_of_event) {
    this.userService.getDataForKnowledPartnerPharma(id_of_event).subscribe(
      data => {
  ...
  send() {
    ...
    console.log(f_d);
    this.userService.send_email_event_for_kp_pharma(f_d).subscribe(
      data => {
        if (data) {
