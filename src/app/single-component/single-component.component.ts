import {APP_ID, Component, Inject, OnInit, PLATFORM_ID, ViewChild} from '@angular/core';
  ...
  get_single_video(slug, catagory) {
    ...
    this.service.get_single_video(slug, catagory).subscribe(
      data => {
        if(data['success']){
  if(data['data']['event']) {
    this.router.navigate(['/' + data['data']['event'] + '/' + data['data']['category'] + '/' + data['data']['slug']]);
    //this.load_data=true;
  } else{
    this.router.navigate(['/event/' + data['data']['category'] + '/' + data['data']['slug']]);
  }
}
