...
@Component({
  selector: 'app-img-presentaion-mailer-event-wise',
  templateUrl: './img-presentaion-mailer-event-wise.component.html',
  styleUrls: ['./img-presentaion-mailer-event-wise.component.css']
})

export class ImgPresentaionMailerEventWiseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pp=false;
    this.wccmm = false;
    this.article_url = 'https://therightdoctors.com/api/beta/imagepresantationlist?key=7xOyNH554tY83cBN7Ktpw3s1y68ql6Eg';
  ...
  send_email() {
    ...
    console.log(f_d);
    this.service.send_img_presentation_email(f_d).subscribe(
      data => {
        console.log(data);
      }
    )
