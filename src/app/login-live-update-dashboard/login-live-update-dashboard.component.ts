...
@Component({
  selector: 'app-login-live-update-dashboard',
  templateUrl: './login-live-update-dashboard.component.html',
  styleUrls: ['./login-live-update-dashboard.component.css']
})
export class LoginLiveUpdateDashboardComponent implements OnInit {
  user:any;
  pass:any;
      ...
      this.auth.login_live_updates(this.user,this.pass).subscribe(
        data => {
          ...
          if(data['success']) {
            ...
            this.router.navigate(['/live-update-dashboard'+"/" +JSON.parse(localStorage.getItem('CurrentUser'))]);
